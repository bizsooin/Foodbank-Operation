...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        graph_size = self.opts.foodbank * self.opts.num_products
        ...
        if dataset is None:
            self.dataset = self.problem.make_dataset(
                foodbank=self.opts.foodbank, num_samples=self.opts.val_size, distribution=self.opts.data_distribution, num_products=self.opts.num_products)
        else:
            self.dataset = dataset
        print("Evaluating baseline model on evaluation dataset")
        cost, _, _, _ = rollout(self.model, self.dataset, self.opts)
    ...
    def wrap_dataset(self, dataset):
        ...
        cost, _,_, _ = rollout(self.model, dataset, self.opts)
    ...
    def epoch_callback(self, model, epoch):
        ...
        cost, _, _, _ = rollout(model, self.dataset, self.opts)
        candidate_vals = cost.cpu().numpy()
